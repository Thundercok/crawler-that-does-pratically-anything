...
class AppleSpotlightDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex) -> None:
        ...
        # 2. Apple Dog-Ear Document Icon (30x36px)
        badge_info = get_ext_badge_info(getattr(item, "file_ext", ""))
        doc_rect = QRectF(rect.x() + 14, rect.y() + (rect.height() - 36) / 2, 30, 36)
        self._draw_apple_document_icon(painter, doc_rect, badge_info["bg"], badge_info["label"])

        # Text Area Boundaries
        text_x = rect.x() + 56
        avail_width = rect.width() - 125

        # 3. File Title (Apple Dark Black #1c1c1e, DemiBold, 13px)
        ...
        # 4. Folder Path Subtitle (Apple Gray #636366, 11px)
        ...
        elided_path = fm_sub.elidedText(file_path, Qt.TextElideMode.ElideMiddle, int(avail_width))
        painter.drawText(int(text_x), int(rect.y() + 41), elided_path)

        # 5. Right Tag (Score % or File Size)
